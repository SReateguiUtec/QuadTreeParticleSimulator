...
static Particle makeParticle(int id, double x, double y, double radius,
                             mt19937 &generator) {
  return {id,    x, y, randomVelocity(generator), randomVelocity(generator),
          radius};
}
...
static vector<Particle> generateUniformParticles(int n, double width,
  ...
  for (int i = 0; i < n; i++) {
    particles.push_back(makeParticle(i + 1, xDistribution(generator),
                                     yDistribution(generator), radius,
                                     generator));
  }
...
static vector<Particle> generateClusterParticles(int n, double width,
                                                 double height, double radius,
                                                 mt19937 &generator) {
  vector<Particle> particles;
  ...
  uniform_real_distribution<double> xCenterDistribution(width * 0.15,
                                                        width * 0.85);
  uniform_real_distribution<double> yCenterDistribution(height * 0.15,
                                                        height * 0.85);
  ...
  for (int i = 0; i < clusterCount; i++) {
    centers.push_back(
        {xCenterDistribution(generator), yCenterDistribution(generator)});
  }

  for (int i = 0; i < n; i++) {
    const auto &center = centers[clusterDistribution(generator)];
    double x = clampPosition(center.first + offsetDistribution(generator),
                             radius, width - radius);
    double y = clampPosition(center.second + offsetDistribution(generator),
...
static vector<Particle> generateHighDensityParticles(int n, double width,
                                                     double height,
                                                     double radius,
                                                     mt19937 &generator) {
  vector<Particle> particles;
  ...
  for (int i = 0; i < denseCount; i++) {
    particles.push_back(makeParticle(i + 1, denseXDistribution(generator),
                                     denseYDistribution(generator), radius,
                                     generator));
  }

  for (int i = denseCount; i < n; i++) {
    particles.push_back(makeParticle(i + 1, worldXDistribution(generator),
                                     worldYDistribution(generator), radius,
                                     generator));
  }
...
vector<Particle> generateParticles(DistributionType distribution, int n,
...
//////////////////////////////
// Funcion minima: reconstruir el QuadTree por frame y comparar contra fuerza
// bruta.
//////////////////////////////

ExperimentResult runExperiment(DistributionType distribution, int particleCount,
  ...
  vector<Particle> particles = generateParticles(
      distribution, particleCount, width, height, radius, 42 + particleCount);
  ...
  for (int frame = 0; frame < frames; frame++) {
    ...
    if (quadStats.collisions != bruteStats.collisions) {
      cout << "Advertencia: resultados distintos en "
           << distributionName(distribution) << " n=" << particleCount
           << " frame=" << frame << ". QuadTree=" << quadStats.collisions
           << ", fuerza bruta=" << bruteStats.collisions << endl;
    }

    totalBuildTime +=
        duration_cast<microseconds>(buildEnd - buildStart).count();
    totalQuadTreeTime +=
        duration_cast<microseconds>(quadEnd - quadStart).count();
    totalBruteForceTime +=
        duration_cast<microseconds>(bruteEnd - bruteStart).count();
  ...
  return {distributionName(distribution),
          particleCount,
          frames,
...
void printExperimentHeader(double width, double height, double radius,
                           int capacity, int frames) {
  cout << "Experimentos QuadTree vs fuerza bruta" << endl;
  cout << "Mundo: " << width << " x " << height << ", radio: " << radius
       << ", capacidad: " << capacity << ", frames por caso: " << frames
       << endl;
  cout << "Los tiempos estan expresados en microsegundos." << endl;
  cout << "Candidatos por particula = comparaciones promedio del QuadTree / "
          "numero de particulas."
       << endl;
  cout << "Nodos visitados, particulas revisadas y candidatos corresponden a "
          "las consultas internas del QuadTree."
       << endl;
  cout << endl;

  cout << left << setw(18) << "Distribucion" << right << setw(16)
       << "Particulas" << setw(10) << "Frames" << setw(30)
       << "Construccion QuadTree" << setw(30) << "Deteccion con QuadTree"
       << setw(32) << "Deteccion con fuerza bruta" << setw(32)
       << "Comparaciones QuadTree" << setw(34) << "Comparaciones fuerza bruta"
       << setw(30) << "Nodos visitados QuadTree" << setw(34)
       << "Particulas revisadas QuadTree" << setw(28) << "Candidatos QuadTree"
       << setw(22) << "Colisiones" << setw(34) << "Candidatos por particula"
       << endl;
}

void printResult(const ExperimentResult &result) {
  cout << left << setw(18) << result.distributionName << right << setw(16)
       << result.particleCount << setw(10) << result.frames << setw(30) << fixed
       << setprecision(0) << result.averageBuildTimeUs << setw(30) << fixed
       << setprecision(0) << result.averageQuadTreeTimeUs << setw(32) << fixed
       << setprecision(0) << result.averageBruteForceTimeUs << setw(32) << fixed
       << setprecision(0) << result.averageQuadTreeComparisons << setw(34)
       << fixed << setprecision(0) << result.averageBruteForceComparisons
       << setw(30) << fixed << setprecision(0)
       << result.averageQuadTreeNodesVisited << setw(34) << fixed
       << setprecision(0) << result.averageQuadTreeParticlesChecked << setw(28)
       << fixed << setprecision(0) << result.averageQuadTreeCandidatesFound
       << setw(22) << fixed << setprecision(0) << result.averageCollisions
       << setw(34) << fixed << setprecision(2)
       << result.averageCandidatesPerParticle << endl;
}

void writeResultsCsv(const string &fileName,
  ...
  for (const ExperimentResult &result : results) {
    file << result.distributionName << ',' << result.particleCount << ','
         << result.frames << ',' << fixed << setprecision(2)
         << result.averageBuildTimeUs << ',' << result.averageQuadTreeTimeUs
