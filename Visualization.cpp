...
static vector<CollisionPair>
findCollisionPairs(const vector<Particle> &particles) {
...
static vector<Particle> generateGalaxyParticles(int particleCount, double width,
                                                double height,
  ...
  for (int i = 0; i < particleCount; i++) {
    ...
    double x = centerX + cos(angle) * (distance + armWave) +
               noiseDistribution(generator);
    double y = centerY + sin(angle) * (distance * 0.72 + armWave) +
               noiseDistribution(generator);
    ...
    particles.push_back(
        {i + 1, clampCoordinate(x, particleRadius, width - particleRadius),
         clampCoordinate(y, particleRadius, height - particleRadius), vx, vy,
         particleRadius});
  }
...
static void writeSvgVisualization(const string &fileName,
                                  const vector<Particle> &particles,
                                  const vector<Rectangle> &boundaries,
                                  const vector<Particle> &queryCandidates,
                                  const vector<CollisionPair> &collisions,
                                  double width, double height, double queryX,
                                  double queryY, double queryWidth,
                                  double queryHeight, double queryRadius) {
  ...
  file << "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"1200\" "
          "height=\"1100\" viewBox=\"0 0 1200 1100\">\n";
  file << "<rect width=\"1200\" height=\"1100\" fill=\"#f8fafc\"/>\n";
  file << "<text x=\"30\" y=\"35\" font-family=\"Arial\" font-size=\"24\" "
          "fill=\"#111827\">Visualizacion QuadTree - particulas, "
          "subdivisiones, consulta y colisiones</text>\n";
  file << "<text x=\"30\" y=\"65\" font-family=\"Arial\" font-size=\"15\" "
          "fill=\"#374151\">Azul: particulas | Naranja: candidatos de la "
          "consulta rectangular | Rojo: particulas en colision | Morado: "
          "region consultada | Lineas grises: nodos del QuadTree</text>\n";
  file << "<g transform=\"translate(50, 80)\">\n";
  file << "<rect x=\"0\" y=\"0\" width=\"" << width << "\" height=\"" << height
  ...
  for (const Rectangle &boundary : boundaries) {
    ...
    file << "<rect x=\"" << x << "\" y=\"" << y << "\" width=\"" << boundary.w
         << "\" height=\"" << boundary.h
         << "\" fill=\"none\" stroke=\"#9ca3af\" stroke-width=\"0.8\"/>\n";
  }

  file << "<rect x=\"" << queryX - queryWidth / 2 << "\" y=\""
       << queryY - queryHeight / 2 << "\" width=\"" << queryWidth
       << "\" height=\"" << queryHeight
       << "\" fill=\"#8b5cf6\" fill-opacity=\"0.08\" stroke=\"#7c3aed\" "
          "stroke-width=\"3\"/>\n";

  for (const CollisionPair &collision : collisions) {
    file << "<line x1=\"" << collision.first.x << "\" y1=\""
         << collision.first.y << "\" x2=\"" << collision.second.x << "\" y2=\""
         << collision.second.y
...
void generateVisualization(double width, double height, double radius,
  ...
  vector<Particle> particles =
      generateParticles(DistributionType::HighDensity, particleCount, width,
                        height, radius, 2026);
  ...
  writeSvgVisualization(fileName, particles, boundaries, queryCandidates,
                        collisions, width, height, queryX, queryY, queryWidth,
                        queryHeight, queryRadius);
  ...
  cout << "Consulta visual -> nodos visitados: " << queryStats.nodesVisited
       ...
       << ", candidatos dentro del radio: " << queryCandidates.size() << endl;
}
...
void generateVisualizationFrames(double width, double height, double radius,
  ...
  Rectangle world{width / 2, height / 2, width, height};
  vector<Particle> particles =
      generateGalaxyParticles(particleCount, width, height, radius);
  ofstream file(fileName);
  ...
  for (int frame = 0; frame < totalFrames; frame++) {
    ...
    file << "    {\n";
    file << "      index: " << frame << ",\n";
    file << "      query: { type: \"rectangle\", x: " << queryX
         << ", y: " << queryY << ", w: " << queryWidth << ", h: " << queryHeight
         << ", radius: " << queryRadius << " },\n";
    ...
    for (size_t i = 0; i < particles.size(); i++) {
      ...
      file << "{id:" << particle.id << ",x:" << particle.x
           << ",y:" << particle.y << ",r:" << particle.radius << ",candidate:"
           << (candidateIds.count(particle.id) > 0 ? "true" : "false")
           << ",colliding:"
           << (collidingIds.count(particle.id) > 0 ? "true" : "false") << "}";
    }
    file << "],\n";
    ...
    for (size_t i = 0; i < collisions.size(); i++) {
      ...
      file << "{a:" << collision.first.id << ",b:" << collision.second.id
           << "}";
    }
    file << "]\n";
    file << "    }";
...
std::string generateVisualizationFramesJson(double width, double height,
  ...
  if (distributionType == 3) {
    ...
    particles =
        generateParticles(static_cast<DistributionType>(distributionType),
  ...
  for (int frame = 0; frame < totalFrames; frame++) {
    ...
    for (const Particle &p : particles) {
      ...
      vector<Particle> neighbors =
          qt.queryNearPoint(p.x, p.y, radius * 2.01, cs);
    ...
    os << "    {\n";
    os << "      \"index\": " << frame << ",\n";
    os << "      \"query\": { \"type\": \"rectangle\", \"x\": " << queryX
       << ", \"y\": " << queryY << ", \"w\": " << queryWidth
       << ", \"h\": " << queryHeight << ", \"radius\": " << queryRadius
       << " },\n";
    ...
    for (size_t i = 0; i < particles.size(); i++) {
      ...
      os << "{\"id\":" << particle.id << ",\"x\":" << particle.x
         << ",\"y\":" << particle.y << ",\"r\":" << particle.radius
         << ",\"candidate\":"
         << (candidateIds.count(particle.id) > 0 ? "true" : "false")
         << ",\"colliding\":"
         << (collidingIds.count(particle.id) > 0 ? "true" : "false") << "}";
    ...
    for (size_t i = 0; i < collisions.size(); i++) {
      ...
      os << "{\"a\":" << collision.first.id << ",\"b\":" << collision.second.id
         << "}";
    }
    os << "],\n";
...
void streamSimulationFrames(double width, double height, double radius,
  ...
  // Send metadata as first message
  {
    ...
    os << "{\"type\":\"init\""
       ...
       << ",\"particleCount\":" << particleCount << ",\"capacity\":" << capacity
...
void streamSimulationLive(std::mutex &mtx,
  ...
  double queryRadius = max(queryWidth, queryHeight) / 2;
  {
    ...
    os << "{\"type\":\"init\""
       << ",\"width\":" << width << ",\"height\":" << height
       << ",\"particleRadius\":" << radius << ",\"capacity\":" << capacity
       << ",\"queryRadius\":" << queryRadius << "}";
  ...
  while (true) {
    ...
    if (particles.empty()) {
      ...
      os << "{\"type\":\"frame\",\"index\":" << frame
         ...
         << ",\"y\":" << height / 2 << ",\"w\":" << queryWidth
         << ",\"h\":" << queryHeight << ",\"radius\":" << queryRadius << "}"
         << ",\"metrics\":{\"nodesVisited\":0,\"particlesChecked\":0,"
            "\"candidates\":0,"
         << "\"collisions\":0,\"quadtreeNodes\":1,\"qtComparisons\":0,"
            "\"bfComparisons\":0,\"frameTimeMs\":0}"
         << ",\"particles\":[]"
         << ",\"boundaries\":[{\"x\":0,\"y\":0,\"w\":" << width
         << ",\"h\":" << height << "}]"
         << ",\"collisions\":[]"
